...
export const ScrollTimeline = ({
  ...
  const getCardVariants = (index: number) => {
    const baseDelay =
      animationOrder === "simultaneous" ? 0 : animationOrder === "staggered" ? index * 0.2 : index * 0.3
    ...
    return {
      ...
      whileInView: {
        ...
        scale: 1,
        rotateY: 0,
        transition: { duration: 0.7, delay: baseDelay, ease: [0.25, 0.1, 0.25, 1.0] as [number, number, number, number] },
      },
  ...
  const getConnectorClasses = () => {
    ...
    if (connectorStyle === "dots") return cn(baseClasses, "w-1 rounded-full")
  ...
  return (
    <div ref={scrollRef} className={cn("relative w-full overflow-hidden", className)}>
      {title && (
        <div className="text-center pb-16 px-4">
          <h2 className="text-3xl md:text-5xl font-bold mb-4 text-[var(--color-text)]">{title}</h2>
          {subtitle && <p className="text-lg text-[var(--color-muted)] max-w-2xl mx-auto">{subtitle}</p>}
        </div>
      )}
          ...
          {progressIndicator && (
            <>
              ...
              <motion.div
                ...
                style={{ top: progressHeight, left: "50%", translateX: "-50%", translateY: "-50%" }}
              >
                <motion.div
                ...
                <div
                  ...
                  className={cn(
                    ...
                    cardAlignment === "alternating"
                      ...
                      : cardAlignment === "left" ? "lg:justify-start" : "lg:flex-row-reverse lg:justify-start"
                  )}
                >
                  {/* Point sur la ligne */}
                  ...
                  >
                    {dateFormat === "badge" ? (
                      <div className="flex items-center mb-2">
                        {event.icon || <Calendar className="h-4 w-4 mr-2 text-[var(--color-primary)]" />}
                        <span className="text-sm font-bold text-[var(--color-primary)]" style={{ fontFamily: "var(--font-display)" }}>
                          {event.year}
                        </span>
                      </div>
                    ) : (
                      <p className="text-lg font-bold text-[var(--color-primary)] mb-2" style={{ fontFamily: "var(--font-display)" }}>
                        {event.year}
                      </p>
                    )}

                    <h3 className="text-xl font-bold mb-1 text-[var(--color-text)]">{event.title}</h3>

                    {event.subtitle && <p className="text-[var(--color-muted)] font-medium mb-2 text-sm">{event.subtitle}</p>}

                    <p className="text-[var(--color-muted)] text-sm leading-relaxed">{event.description}</p>
                  </motion.div>
                </div>
              )
            })}
